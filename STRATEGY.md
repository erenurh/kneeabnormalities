...
**Progress (as of 2026-08-27):** LB 0.872 → **0.922** in 8 days (v2 single model: 24 slices x 320px x BiGRU+attention-MIL+slot-attention on distill-r2 targets; 0.92 efficiency threshold PASSED, chasing 0.94-0.95). Proven levers, in measured order of impact: distillation from the 5-fold ensemble teacher (+0.02 single-model), longer training on clean distill targets (+0.008), distill round 2 / stronger teacher (+0.003 LB, +0.007 target-gold), fold ensembling (+0.016), own labels (+0.008), resolution 256→320 (+0.002), slice count (+0.001). Assets: five 320px distill-trained fold models (ensemble members), r2 targets at gold 0.9237.

**Doctrine (2026-08-26, team lead): architecture-first, distill-last.** All work is now ordered as (A) raise the architecture to winner spec → (B) strengthen supervision (label v5, confident learning, Phase B gold finetune) → (C) build the best teacher ensemble and run ONE distillation pass into the efficiency model. Distillation is a compression step, never a growth engine (measured: r1 +0.02 → r2 +0.003 → fold-ensemble 0.913 < single 0.915). Target: **0.94-0.95**.

**Method revision (2026-08-26, user-approved):** public LB top is 0.952 with a dense pack at 0.944-0.948 — 0.94+ is achievable and our gap (~0.03) is structural, not closable by more distillation (r1 +0.02 → r2 +0.003; self-distillation adds no new information and is converging). Distill round 3 cancelled; distillation is retained only as the final compression step into the efficiency model. New target: **0.94-0.95**. Priority order of unbuilt levers: (1) slice count 15→24 at 320px (new 24-slice cache; single-variable test, launched); (2) aggregation to full spec — BiGRU + attention-MIL over slices, cross-series attention (§3.4, never built; every RSNA winner uses it); (3) label engine v5 targeting weak columns (MCL .78, Synovitis .79, PF OA .80 drag the macro); (4) Phase B gold-58 head finetune + confident-learning cleanup; (5) flip-TTA and aux segmentation head.
