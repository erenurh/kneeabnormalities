...
**Progress (as of 2026-08-23):** LB 0.872 → **0.912** in 5 days (single distilled 320px model — efficiency floor ~0.915 within 0.003). Proven levers, in measured order of impact: distillation from the 5-fold ensemble teacher (+0.02 single-model), longer training on clean distill targets (+0.008), fold ensembling (+0.016), own labels (+0.008), resolution 256→320 (+0.002), slice count (+0.001). Next: distill round 2 (retrain folds on distill targets -> stronger teacher -> re-distill at 320px), label v5 (Qwen3-14B), efficiency runtime measurement (320px inference cost vs <15 min budget).
