...
**Progress (as of 2026-09-06):** LB 0.872 → **0.922** (see EXPERIMENTS.md #1-25; #18-25 were run 08-23→08-29 and only reconciled into this repo on 08-30 after a session gap with no Kaggle API access). Proven levers, in measured order of impact: attention-MIL aggregation (BiGRU over slice triplets + attention over series slots, replacing mean-pool) **+0.007 LB (new best lever)**, distillation from the 5-fold ensemble teacher (+0.02 single-model), longer training on clean distill targets (+0.008), fold ensembling on the plain-320 arch (+0.016, but *negative* at the v2/attention arch — see #20), own labels (+0.008), 320px resolution alone (no win over 256px without the arch change — see #18). Distill round 2 (retrain folds on round-1 distill targets → stronger teacher) shipped and is the label version behind #18-25. Closed thread: v2 arch at 384px (#25-26) — tried both plain batch-1 and a batch-matched (grad-accum) rerun, neither closed the gap to the 320px v2 run's soft-OOF at the same 10-epoch budget; deprioritized, not a submission candidate without a much bigger (unproven) epoch/LR investment. Current best remains the 320px v2 model (LB 0.922). **09-06 update:** runtime harness finally measured (#29): the v2 pipeline is I/O-bound (GPU forward 0.15 s/study); sequential inference would take ~47 min on the hidden test, a 4-process preprocessing pool brings it to ~9 min — the pool alone is worth ≈0.03 AUC-equivalent on the efficiency score. `submit-v2-fast` (same 0.922 checkpoint, pooled, T4/fp16) is prepared and awaits submission approval; v2 fold-4 is training (completes the v2 5-fold OOF for confident-learning / the final teacher). Week plan: (1) submit-v2-fast once approved, (2) per-finding push on the weak four via supervision (label v5 is blocked at Qwen3-14B fp16 ≈ 29.5 GB > 2×T4 without quantization; fallback = Qwen3-8B thinking mode on the four weak findings only, gold-gated), (3) a genuinely diverse second backbone (ConvNeXt-T, v2 aggregation) as the main-LB ensemble member — exp-17/20 showed same-family members don't blend.
...
**Reality check (2026-09-07, from the live leaderboards + the top public kernels):**
- Public LB: 3,254 teams; top-1 0.954, top-10 0.950, top-50 0.944, top-100 0.941; **1,036 teams ≥ 0.93; our 0.922 is rank 1,183**. Efficiency LB (Kaggle-published `ryanholbrook/rsna-knee-abnormalities-efficiency-lb`): top-3 = 0.952 / 0.948 / 0.944 with fast runtimes; **we are efficiency rank 61 at 0.922**. The 0.915 "efficiency floor" in this file is obsolete.
- Public-LB macro-AUC noise ≈ 0.005 sd (≈390 studies); every +0.002/+0.003 step we logged since exp-13 was inside noise. Gold-58 cannot rank prompt or architecture variants per finding either (exp-33/35).
- 0.94 arithmetic (from the exp-27 per-finding anchor): the eight strong findings are .94-.98 and saturated; lifting the five weak ones (Synovitis .839, Lat OA .845, PF OA .876, Lat Men .872, Contusion .906) to .92 gives macro 0.944. There is no other route.
- What the 0.936-0.95 public kernels are: a cascade of 20 DINOv2-S members + a 5-fold 16-channel ViT/timm arm + frozen RadImageNet-ResNet50 attention heads + 4 fully fine-tuned CoAtNet-2@384 arms + an 88-feature linear rank calibrator, all rank-blended over hours on 2×T4 — main-LB material only. **The best public single model (Raptor CoAtNet-2@384, 5 slots/64 slices, span 6-94%, per-finding attention, soft LLM labels, all 4,349 studies) is 0.924 LB / 0.9167 gold — i.e. our v2 (0.922 / 0.9218) is at parity with the best public single model.** Their author reports blends over it bought only +0.001 on LB.
- Transferable single-model levers we do not have yet, in the public authors' own priority: (1) per-finding attention queries over windows and slots instead of one shared attention ("the part that matters most"); (2) fluid-sensitive AND non-fluid series as separate slots per plane (5-6 slots); (3) per-target window pooling (max/top-2 for focal findings). Our slice span and mm-crop already match theirs.
- Consequence for the prize plan: with ~19 GPU-h/week left ×5 weeks, a 0.94+ single model is not a credible target; ~0.93 is (levers 1-3 + possibly a CoAtNet-2 backbone). For the main LB, the public 0.95 stack is the floor for every team, so our own ensemble adds nothing unless it beats that stack.

**Status 09-13:** DINOv2-S backbone swap closed after two runs (partial fine-tune gold 0.9156 < v3 band; full fine-tune at 3e-5 diverged, #59-61). Every single-model lever tried since 09-07 (per-finding head, 6 slots, 32 slices, distill r3, seeds, DINO backbone) lands in 0.916-0.928 gold / 0.922-0.923 LB. The efficiency entry stands at v3 (0.923, ~7 min). Remaining realistic single-model moves: a ViT fine-tune done properly (warmup + LLRD, ~8 h, unproven) or a CoAtNet-2@384 full fine-tune (~13 h+, public evidence says ≈0.924 alone). Neither is a 0.935 path on free quota; the honest expectation for the efficiency prize is now below 5%.

**Status 09-12:** v3 on LB = 0.923 (v2 0.922); a second v3 seed scores 0.9217 on gold vs 0.9277 — the head's gold gain was seed noise. Everything in the EffNetV2-S 2.5D family (v2, v3, r3 targets, 6 slots, seeds) sits at 0.92-0.93 and neither gold-58 nor the public LB can order it. Open: 32-slice v3 (running). Honest position: the 0.935 target needs a lever outside this family (backbone/pretraining, e.g. DINOv2-S or CoAtNet-2 fine-tune at 336-384), not more head/slot/target variants.

**Plan revision status (evening 09-07):** step 1 done — per-finding query-attention head (v3) is the new baseline: gold-58 0.9277 vs 0.9218 (attributed to the head, not the r3 targets: r3-target v2 = 0.9224); lateral meniscus .872→.927. Step 2 (6 FS/non-FS slots at 15 slices) measured 0.9203 → not adopted. Step 3 (speed) done: pool8 + raw InstanceNumber ordering = ~7 min at hidden size, bit-identical volumes. Next candidates: 32-slice cache for v3 (more depth per slot; I/O ~9 min), 2-seed average, then a CoAtNet-2 backbone swap if budget remains. `submit-v3-fast` awaits approval.

**Plan revision (2026-09-07, team lead: go):** target = **efficiency track, ~0.935 AUC at ≤4 min** (the efficiency LB ranks a fast 0.936 above a slow 0.952). Sequence: (1) per-finding query-attention head (`train-v3`, single variable); (2) 6 slots FS+non-FS per plane (cache building on CPU); (3) speed pass: fewer slices / lower resolution / more DICOM parallelism measured on the runtime harness, keeping whatever costs <0.003 AUC; (4) optional CoAtNet-2 backbone swap if budget remains. Every step is judged on LB (noise 0.005) plus the gold-58 anchor, not on 0.003 moves. Weekly budget ~19 GPU-h.
