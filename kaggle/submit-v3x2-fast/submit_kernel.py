...
and runs the two v3 seeds (42, 43) and averages their probabilities (train-v3, train-v3-s43).
...
CKPTS = [sorted(INPUT.rglob("effv2s_v3_all.pt"))[0], sorted(INPUT.rglob("effv2s_v3_s43_all.pt"))[0]]
...
def forward_study(nets, pre):
    ...
    with torch.no_grad(), torch.amp.autocast(DEV if DEV == "cuda" else "cpu"):
        p = sum(torch.sigmoid(n(x, m, s)).float() for n in nets) / len(nets)
    return p.cpu().numpy()[0].tolist()
...
def main():
    ...
    for ck in CKPTS:
        n = Net().to(DEV)
        n.load_state_dict(torch.load(ck, map_location=DEV))
        n.eval()
        nets.append(n)
    rows = []
