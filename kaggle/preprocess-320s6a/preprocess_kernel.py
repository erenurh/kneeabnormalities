...
6-SLOT variant (public Raptor/DINO recipe): fluid-sensitive AND non-fat-sat
series as separate slots for each plane (Sag-FS, Sag-nonFS, Cor-FS, Cor-nonFS,
Ax-FS, Ax-nonFS), 16 slices each (6x16 = 96 slices/study, same I/O as the
4x24 cache). A slot with no matching series stays zero + masked (no duplicate
series across slots; an FS slot may fall back to any unused series of that
plane, a non-FS slot never does). Reads N_SLICES evenly spaced position-sorted slices,
...
def pick_series(g):
    ...
    No series is used twice; FS slots may fall back to any unused series of
    the plane, non-FS slots stay empty if no non-FS series exists."""
    chosen, used = [], set()
    for plane, want_fs in SLOTS:
        g_free = g[~g["SeriesInstanceUID"].isin(used)]
        cand = g_free[(g_free["derived_plane"] == plane) & (g_free["fatsat"] == want_fs)]
        if not len(cand) and want_fs:
            cand = g_free[g_free["derived_plane"] == plane]
        cand = cand.sort_values(["n_slices", "SeriesInstanceUID"], ascending=[False, True])
        pick = cand["SeriesInstanceUID"].iloc[0] if len(cand) else None
        if pick is not None:
            used.add(pick)
        chosen.append(pick)
    return chosen
