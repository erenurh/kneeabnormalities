...
| 23 | 08-26 | v2: 24sl x 320px x BiGRU+attention-MIL+slot-attention (combines exp-21+22; batch 2, LR 1e-4, r2 targets). Trained clean 5.06h, ep9 loss 0.4136 (best of any run) → **LB 0.922** — new overall best (+0.007 over 0.915). Architecture-first validated: one combined structural change beats two distill rounds (+0.003 total) | | | **0.922** |
| 24 | 08-29 | v2 at 384px (single variable vs 0.922: SIZE 320→384, batch 1, LR 5e-5). Trained clean 6.6h/10ep, ep9 loss **0.4105 — best of any run** (v2: 0.4136); in-train AUC not comparable across batch sizes. Awaiting submission approval | | | pending |
| 25 | 08-29 | v2 fold models on r2 targets (Phase C teacher members): f0 .944/4.07h, f1 .941/3.85h, f2 .9395/3.68h, f3 running, f4 next week | | | |
| 20 | 08-26 | 5-fold 320px distill-r2 rank ensemble → **LB 0.913** (< single 0.915). Confirms exp-12/17: distillation already absorbs the fold-ensemble's information; fold members (80% data each) blend below the all-data student. Main-LB line needs genuinely diverse members, not same-family folds | | | 0.913 |
| 21 | 08-26 | Slices 15→24 @320px (new 24sl cache, single variable vs 0.915; batch 2, LR 1e-4). Trained clean (4.36h, r2 targets); in-train ep9 0.9888 vs baseline 0.9929 (smaller batch trains slower on this biased metric) — LB needed | | | awaiting LB |
| 22 | 08-26 | Aggregation to spec: BiGRU + attention-MIL over triplets + masked slot attention (replaces plain means; single variable vs 0.915). Trained clean (2.85h, r2 targets); in-train ep9 0.9920 vs baseline's 0.9929 (biased metric ~equal) — LB needed for the real read | | | awaiting LB |
